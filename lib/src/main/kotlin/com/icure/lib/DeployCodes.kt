...
suspend fun deployCodes(api: CardinalBaseSdk, codes: List<Code>, local: Boolean = false, regexFilter: String? = null, echo: (String) -> Unit = { println(it) }) {
    ...
    groups.forEach { groupId ->
        try {
            ...
            val existingCodes =
                (if (!local) codeApi.inGroup.getCodes(groupId, codes.map { c -> c.id }).map { it.entity } else codeApi.getCodes(codes.map { c -> c.id }))
                    .associateBy { c -> c.id }
            ...
            codesToAdd.chunked(100).forEach { chunk ->
                if (!local) {
                    codeApi.inGroup.createCodes(chunk.map { GroupScoped(it, groupId) })
                } else {
                    codeApi.createCodes(chunk)
                }
            }
            codesToUpdate.chunked(100).forEach { chunk ->
                if (!local) codeApi.inGroup.modifyCodes(chunk.map { GroupScoped(it, groupId) }) else codeApi.modifyCodes(chunk)
            }
